...
SELECT 
    ...
    (SELECT Address 
     ...
     WHERE ED.EmployeeID = E.EmployeeID) AS Address
FROM Employees E;
...
SELECT 
    ...
    (SELECT BirthDate 
     FROM EmployeeDetails ED 
     WHERE ED.EmployeeID = E.EmployeeID AND ED.MaritalStatus = N'Неодружений') AS BirthDate
FROM Employees E
WHERE EXISTS (
    SELECT 1 
    FROM EmployeeDetails ED 
    WHERE ED.EmployeeID = E.EmployeeID AND ED.MaritalStatus = N'Неодружений'
);
...
SELECT 
    ...
    (SELECT BirthDate 
     FROM EmployeeDetails ED 
     WHERE ED.EmployeeID = E.EmployeeID) AS BirthDate
FROM Employees E
WHERE EXISTS (
    SELECT 1 
    FROM EmployeePositions EP 
    WHERE EP.EmployeeID = E.EmployeeID AND EP.Position = N'Менеджер'
);